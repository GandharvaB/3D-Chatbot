...
export default function FaceAnimation({ morphMeshes }) {
  useFrame((state, delta) => {
    ...
    if (store.avatarState === AVATAR_STATES.SPEAKING && analyser) {
      ...
      const lowEnergy = Math.min(1, getEnergy(VISEME_BANDS.low.start, VISEME_BANDS.low.end) * 2.5);
      const midEnergy = Math.min(1, getEnergy(VISEME_BANDS.mid.start, VISEME_BANDS.mid.end) * 2.0);
      const highEnergy = Math.min(1, getEnergy(VISEME_BANDS.high.start, VISEME_BANDS.high.end) * 2.5);
